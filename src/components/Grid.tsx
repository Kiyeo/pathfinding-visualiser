...
export default () => {
  ...
  const handleMouseEvent = (e: React.BaseSyntheticEvent<PointerEvent, EventTarget & HTMLDivElement, EventTarget>) => {
    if (e) {
    ...
    <Grid rows={gridDimensions.rows} columns={gridDimensions.columns}>
      {grid.map((rows: any, rowIndex: any) =>
        rows.map((node: INode, colIndex: any) => <Node key={`${rowIndex} ${colIndex}`} node={node}
          handleMouseEvent={(e: React.BaseSyntheticEvent<PointerEvent, EventTarget & HTMLDivElement, EventTarget>) =>
            handleMouseEvent(e)
          }
        />)
      )}
